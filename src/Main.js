...
function Main({
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch(
        "https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyCS6lW31lKioRcxiUsRAtkSrVTesio_T4M&sort=style"
      );
  ...
  const filteredFonts = fonts.filter(font => {
    return (font.family.toLowerCase().indexOf(searchFonts.toLowerCase()) !== -1 &&
      font.category.toLowerCase().indexOf(searchCategorys.toLowerCase()) !== -1 &&
      font.subsets.toString().toLowerCase().indexOf(searchSubsets.toString().toLowerCase()) !== -1
    );
        ...
        >
           {displayedFonts.map((font, index) => (
            <FontCard
              ...
              categoryName={font.category}
              category={font.category}
              familyVariants={font.variants}
              ...
              fontSize={fontSize}
              addFont={addFont}
              addFamilyFont={addFamilyFont}
              addFamilyVariant={addFamilyVariant}
